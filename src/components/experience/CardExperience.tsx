    ...
    <div className="bg-[rgb(36,36,36)] relative sm:w-4/5 animate-fade-right flex flex-col mx-3 sm:mx-auto rounded-lg py-4 px-6 md:px-16 md:py-8 my-2">
      ...
      <div className="w-full md:w-2/3 ">
        <h3 className="text-white text-lg md:text-2xl ">
          <span className="font-bold text-[#fc31f9]">
            {experience.resaltado}{" "}
          </span>
          {experience.titulo}
        </h3>
      ...
      <div className="w-full md:hidden">
        {experience.periodo ? (
          <div className="flex gap-2 my-2 items-center">
            ...
            <p className="text-[rgb(146,146,146)] text-xs">
              {experience.periodo.inicio} - {experience.periodo.fin}
            </p>
          </div>
        ) : (
          <div className="flex gap-2 my-2 items-center">
      ...
      <div className="flex flex-col mt-6 w-5/6">
        ...
        {Array.isArray(experience.proyectos) &&
          experience.proyectos.length > 0 && (
            <div className="mt-6 space-y-4">
              <h4 className="text-sm md:text-base font-bold text-white/90">
                Proyectos destacados
              </h4>

              {experience.proyectos.map((p, i) => (
                <div key={i} className="space-y-2">
                  ...
                  {p.bullets?.length > 0 && (
                    <ul className="list-disc list-outside marker:text-pink-500 marker:text-base ml-5 text-xs md:text-sm leading-relaxed">
                      {p.bullets.map((b, idx) => (
                        <li key={idx}>{b}</li>
                      ))}
                    </ul>
        ...
        {experience.habilidades !== null && (
          <>
            <p className="mt-4 font-bold text-lg">💡 Habilidades adquiridas:</p>
            <ul className="list-none text-xs md:text-sm">
              {experience.habilidades.map((item, index) => (
                <li
                  key={index}
                  className="before:content-['✔'] before:mr-2 indent-2 md:indent-8"
                >
                  {item}
                </li>
              ))}
